...
export function MetricCard({
  ...
  if (!hasValue) {
    return (
      <div
        data-testid="eb-kpi-card"
        className="bg-[#f6f8fb] rounded-lg border-t-4 border-t-[#cfd7e2] border border-gray-100 border-t-4 p-2.5 self-start min-w-0"
      >
        <h3 className="text-[11px] font-medium text-gray-500 uppercase tracking-wide break-words">{label}</h3>
        ...
        {open && (
          <div className="mt-1.5 text-[11px] text-gray-500 space-y-1 border-t pt-1.5">
            <p className="break-words">Chưa xác định từ hồ sơ tải lên.</p>
            {metric.formula && <p className="break-words">Công thức: {metric.formula}</p>}
  ...
  return (
    <div
      data-testid="eb-kpi-card"
      className={`bg-white rounded-lg border border-gray-100 border-t-4 shadow-sm p-4 min-w-0 ${TONE_BORDER[tone]}`}
    >
      <h3 className="text-xs font-medium text-gray-500 uppercase tracking-wide break-words">{label}</h3>
      {/* A large VND figure ("128.061.897.765 VND") is wider than a
          4-column KPI grid cell at text-2xl — without min-w-0 on this
          card (grid items default to min-width:auto, refusing to shrink
          below their content) and break-words on the value itself, the
          number overflowed the card and visually bled into the next one.
          Wrapping to a second line keeps every digit visible; this must
          never use overflow-hidden/truncate, which would silently hide
          part of a monetary figure. */}
      <p className="text-xl font-bold mt-1 text-msb-navy break-words leading-tight">
        {formatMetricValue(metric.value, unit)} {unit}
      </p>
      {note && <p className="text-xs text-gray-500 mt-1 break-words">{note}</p>}
